...
<link href="<%=path%>/js/calendar-blue.css" rel="stylesheet"
	type="text/css" />
<style>
p {
	text-indent: 2em;
...
<body>
	<form id="form1" name="form1" class="word_darkGrey" method="post"
		onsubmit="return InputM();" action='<%=path%>/testAddServlet'>
		<div class="text2">
			<p>
			...
			<p>
				...
				<c:forEach var="stuclass" items="${classesList}">
					<input type="checkbox" name="classCheck" value="${stuclass.id}"
						checked="checked" />${stuclass.name}
				</c:forEach>
			</p>
			<p align="center">

<%@page import="com.ouc.onlinexam.po.Question"%>
	...
	<div id="main_content">
		<div>
			<form id="form1" name="form1" method="post"
				action="<%=path %>/studentTestServlet?&testId=${test.id}">
				<div id="fd">
				...
				<c:forEach var="que" items="${quesList}" varStatus="status">
					...
					<p>D.${que.choiceD }</p>
					<p>
						A.<input type="radio" name="ques_${que.id}" value="A" /> B.<input
							type="radio" name="ques_${que.id}" value="B" /> C.<input
							type="radio" name="ques_${que.id}" value="C" /> D.<input
							type="radio" name="ques_${que.id}" value="D" />
					</p>
				</c:forEach>
